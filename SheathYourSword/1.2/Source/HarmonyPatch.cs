...
namespace SYS
{
    ...
    public static class DrawPatch
    {
        static DrawPatch()
        {
            Harmony  harmonyInstance = new Harmony("com.SYS.rimworld.mod"); 
    ...
    public static class DrawEquipment_WeaponBackPatch
    {
                ...
                if (compW!=null)
                {
                    switch (___pawn.Rotation.AsInt)
                    {
                        ...
                        case 2:
                            ...
                            drawLoc.y += drawYPosition;
                            DrawEquipmentAiming(___pawn.equipment.Primary, drawLoc, compW.Props.southOffset.angle,true);
                            break;
                        case 3:
                            drawLoc += compW.Props.westOffset.position;
                            drawLoc.y += drawYPosition;
                            DrawEquipmentAiming(___pawn.equipment.Primary, drawLoc, compW.Props.westOffset.angle,true);
                            break;
                        default:
                            break;
        ...
        public static void DrawEquipmentAiming(Thing eq, Vector3 drawLoc, float aimAngle,bool isFlip=false)
        {
            ...
            if(isFlip)
            {
                Graphics.DrawMesh(MeshPool.plane10Flip  , drawLoc, Quaternion.AngleAxis(aimAngle, Vector3.up), matSingle, 0);
            }
            else
            {
